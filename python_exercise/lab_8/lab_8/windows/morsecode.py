...
def is_validated_english_sentence(user_input):
    ...
    user_input_to_list_tmp = copy.deepcopy(user_input_to_list)
    for x in user_input_to_list:
        if(x.isdigit()):
            return False
    ...
    for i in user_input_to_list:
        if(i == '.' or i == ',' or i == '!' or i == '?'):
            user_input_to_list_tmp.remove(i)

    if(user_input_to_list_tmp.count(" ") == len(user_input_to_list_tmp)):
        return False
...
def is_validated_morse_code(user_input):
    ...
    bool_first_condition = False
    bool_second_condition = False
    for i in user_input_to_list:
        for j in i:
            if(j != '-' and j != '.' and j != ' '):
                bool_first_condition = True
                break
    for i in user_input_to_list:
        for j in morse_code_values:
            if (i == j):
                bool_second_condition = False
                break
            else:
                bool_second_condition = True

    if(bool_first_condition + bool_second_condition):
        return False
    else:
        return True
    # ==================================
...
def get_cleaned_english_sentence(raw_english_sentence):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    raw_english_sentence = list(raw_english_sentence)
    while(raw_english_sentence.count(".") != 0):
        raw_english_sentence.remove(".")
    while(raw_english_sentence.count(",") != 0):
        raw_english_sentence.remove(",")
    while(raw_english_sentence.count("!") != 0):
        raw_english_sentence.remove("!")
...
def decoding_character(morse_character):
    ...
    if(morse_character == ''):
        return ' '
    for i in list(morse_code_dict.keys()):
...
def encoding_sentence(english_sentence):
    ...
    cleaned_english_sentence_to_list = list(cleaned_english_sentence)

    result = ""
    for i in cleaned_english_sentence_to_list:
        encoded = encoding_character(i)
        if(encoded == ' '):
