...
def get_number_of_lines(filename):
    ...
    if splited_contents[len(splited_contents)-1] =='':
        return len(splited_contents)-1
    else:
...
def get_number_of_target_words(filename, target_words):
    ...
    # 2750
    # ===Modify codes below=============
    target_word = target_words.upper()
    contents = get_file_contents(filename)
    contents = contents.upper()

    return contents.count(target_word)
